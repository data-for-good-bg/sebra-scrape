...
import pandas as pd
import numpy as np
from settings import unparsable_files_drive_id
from time import sleep

class SebraParser:
    def __init__(self, parent_folder):
        self.parent_folder = parent_folder
        self.unparsed_data_folder_id = unparsable_files_drive_id
    ...
    def find_organizations_start_row(self, excel_file_path, gdrive_manager):
        org_start_phrases = [
        ...
        try:
            ...
            name = excel_file_path.split('/')[-1]
            gdrive_manager.GDM.upload_to_target_drive(excel_file_path, name, self.unparsed_data_folder_id)
            os.remove(excel_file_path)
            print(f'File {name} cannot be parsed, error message: {e}')
            raise
        return mask[mask].index[0]
    
    def find_operations_blocks(self, org_start_row, excel_file_path, gdrive_manager):
    ...
    def get_organization_name_and__period(self, b_start, excel_file_path, gdrive_manager):
    ...
    def get_organization_operations_blocks(self, org_op_blocks, excel_file_path, gdrive_manager):
        ...
        for b_start, b_end in org_op_blocks:
            ...
            org_name, start_date, end_date = self.get_organization_name_and__period(b_start, excel_file_path, gdrive_manager)
    ...
    def check_sums(self, ops_df, general_block_df, excel_file_path, gdrive_manager):
    ...
    def run_parser(self, gdrive_manager):
        # Get all excel files
        ...
        for i, f in enumerate(excel_files):
            ...
            # Read data
                ...
                org_start_row = self.find_organizations_start_row(f,gdrive_manager)
                
                # get the start and end rows for all blocks of operations
                op_blocks = self.find_operations_blocks(org_start_row, f, gdrive_manager)
                ...
                ops_df = self.get_organization_operations_blocks(op_blocks['organizations'], f, gdrive_manager)
                
                # check if the sums for all opearions by organization match the general totals
                self.check_sums(ops_df, general_block_df, f, gdrive_manager)
