...
class SebraParser:
    def __init__(self, parent_folder):
        self.excel_file_path = parent_folder

        def read_into_df(self):
            self.df = pd.read_excel(self.excel_file_path)

        # def parse_sebra_excel_file_to_pandas(self):
        def find_organizations_start_row(self):
            org_start_phrases = [
        ...
        def find_operations_blocks(self, org_start_row):
            ...
            for i in range(len(op_blocks)):
                ...
                assert b_start < b_end, f'Block start index ({b_start}) should be smaller that the end index ({b_end}). File "{self.excel_file_path}".'
                if i > 0:
                    _, prev_b_end = op_blocks[i - 1]
                    assert prev_b_end < b_start, f'Block end index ({prev_b_end}) of the previous block should be smaller that the start index of the next block ({b_start}). File "{self.excel_file_path}".'
                assert not (b_start < org_start_row and org_start_row < b_end), f'The starting row for organizations shouldn\'t be within an operations block. File "{self.excel_file_path}".'
        ...
        def get_general_totals(self, general_op_block):
            ...
            if not round(sum_row['Operations Amount (BGN)'], 2) == round(block_df['Operations Amount (BGN)'].sum(), 2):
                print(f"Warning: The sums of \"Operations Amount (BGN)\" do not match for block ({b_start}, {b_end}). Expected value \"{round(sum_row['Operations Amount (BGN)'], 2)}\". Calculated value \"{round(block_df['Operations Amount (BGN)'].sum(), 2)}\". File \"{self.excel_file_path}\".")

            return block_df

        def get_organization_name_and__period(self, b_start):
            ...
            if isinstance(block_header[2], str):
                ...
                assert isinstance(block_header_1[0], str), f'The first column does not contain the organization name around row {b_start} in file "{self.excel_file_path}".'
                org_name = block_header_1[0] + ' ' + org_name
                assert isinstance(block_header_1[2], str), f'The third column does not contain the period string around row {b_start} in file "{self.excel_file_path}".'
                period = block_header_1[2].split(' ')
            assert period[0] == 'Период:', f'Field for time period should start with string "Период". File "{self.excel_file_path}".'
            start_date = period[1]
            end_date = period[3]
            return org_name, start_date, end_date

        def get_organization_operations_blocks(self, org_op_blocks):
            ...
            for b_start, b_end in org_op_blocks:
                ...
                assert not org_df.iloc[:, 1].isna().any(), f'There are empty cells within the operations block between rows ({b_start}, {b_end}). File "{self.excel_file_path}".'
                ...
                org_name, start_date, end_date = self.get_organization_name_and__period(b_start)
                ...
                if not round(sum_row['Operations Amount (BGN)'], 2) == round(org_df['Operations Amount (BGN)'].sum(), 2):
                    print(f"Warning: The sums of \"Operations Amount (BGN)\" do not match for block ({b_start}, {b_end}). Expected value \"{round(sum_row['Operations Amount (BGN)'], 2)}\". Calculated value \"{round(org_df['Operations Amount (BGN)'].sum(), 2)}\". File \"{self.excel_file_path}\".")

                org_op_blocks_dfs.append(org_df)
            return pd.concat(org_op_blocks_dfs).reset_index().drop(columns=['index'])

        def check_sums(self, ops_df, general_block_df):
            ...
            if not (sum_check_df['Operations Amount (BGN)_x'].round(2) == sum_check_df['Operations Amount (BGN)_y'].round(2)).all():
                print(f'Warning: Some sums do not match Operations Amount (BGN). File "{self.excel_file_path}".')
                
               
        def run_parser(self):
            # get the row that indicates the start of the organizations section
            org_start_row = find_organizations_start_row(self.df)
            
            # get the start and end rows for all blocks of operations
            op_blocks = find_operations_blocks(self.df, org_start_row)
            ...
            general_block_df = get_general_totals(self.df, op_blocks['general'][0])
            
            # parse all operations by organization in the file
            ops_df = get_organization_operations_blocks(self.df, op_blocks['organizations'])
